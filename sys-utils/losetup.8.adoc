...
*--direct-io*[**=on**|*off*]::
Enable or disable direct I/O for the backing file.  The default is *off*.  Specifying either `--direct-io` or `--direct-io=on` will enable it.  But, `--direct-io=off` can be explicitly specified to disable it.
